    ...
    private final class MyUi implements PatientChartController.Ui {
		...
		public void setPatient(Patient patient) {
	        ...
	        if (patient.assigned_location != null) {
	            LocationSubtree patientZone = locationTree.getZoneForUuid(patient.assigned_location.uuid);
	            LocationSubtree patientTent = locationTree.getTentForUuid(patient.assigned_location.uuid);
	            zoneName = (patientZone == null) ? zoneName : patientZone.toString();
	            tentName = (patientTent == null) ? tentName : patientTent.toString();
	        }

	        mPatientFullNameView.setText(patient.given_name + " " + patient.family_name);
	        mPatientIdView.setText(patient.id);

	        if ("years".equals(patient.age.type)) {
	        	mPatientAgeView.setText(patient.age.years + "-year-old ");
	        } else {
	        	mPatientAgeView.setText(patient.age.months + "-month-old ");
	        }

	        mPatientGenderView.setText(patient.gender.equals( "M" ) ? "Male" : "Female");
	        mPatientLocationView.setText(zoneName + "/" + tentName);

	        int days = Days
	                .daysBetween(new DateTime(patient.admission_timestamp * 1000), DateTime.now())
	                .getDays();
	        switch (days) {
	            case 0:
