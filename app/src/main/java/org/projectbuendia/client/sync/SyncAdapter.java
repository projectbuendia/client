...
import org.projectbuendia.client.sync.controllers.ChartsSyncController;
import org.projectbuendia.client.sync.controllers.ConceptsSyncController;
import org.projectbuendia.client.sync.controllers.FormsSyncController;
import org.projectbuendia.client.sync.controllers.LocationsSyncController;
import org.projectbuendia.client.sync.controllers.ObservationsSyncController;
import org.projectbuendia.client.sync.controllers.OrdersSyncController;
import org.projectbuendia.client.sync.controllers.PatientsSyncController;
import org.projectbuendia.client.sync.controllers.SyncController;
import org.projectbuendia.client.sync.controllers.UsersSyncController;
...
public class SyncAdapter extends AbstractThreadedSyncAdapter {
    ...
    public enum SyncPhase {
        SYNC_USERS(R.string.syncing_users, new UsersSyncController()),
        SYNC_LOCATIONS(R.string.syncing_locations, new LocationsSyncController()),
        SYNC_CHART_ITEMS(R.string.syncing_charts, new ChartsSyncController()),
        SYNC_CONCEPTS(R.string.syncing_concepts, new ConceptsSyncController()),
        SYNC_PATIENTS(R.string.syncing_patients, new PatientsSyncController()),
        SYNC_OBSERVATIONS(R.string.syncing_observations, new ObservationsSyncController()),
        SYNC_ORDERS(R.string.syncing_orders, new OrdersSyncController()),
        SYNC_FORMS(R.string.syncing_forms, new FormsSyncController());

        @StringRes
        private final int message;
        private final SyncController syncController;

        SyncPhase(@StringRes int message, SyncController controller) {
    ...
    @Override public void onPerformSync(
        ...
        try {
            ...
            final float progressIncrement = 100 / phases.size();
            ...
            for (SyncPhase phase : SyncPhase.values()) {
                ...
                reportProgress((int) (stagesComplete * progressIncrement), phase.message);

                phase.syncController.sync(syncResult, provider, mContentResolver);
